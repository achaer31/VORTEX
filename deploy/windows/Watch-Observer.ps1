...
if ($env:VORTEX_BASELINE_STATUS -ne 'passed_reviewed') {
...
while (-not (Test-Path $StopFile)) {
    & $PythonExe (Join-Path $RepoRoot 'live\observe_v02.py') --observe 2>&1 | Out-File -Append -Encoding utf8 $LogFile
    $Code = $LASTEXITCODE
